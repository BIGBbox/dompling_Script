...
function notify(_number) {
  return new Promise((resolve) => {
    try {
      ...
      var three =
        TCash || TSubsidy ? `【其他奖励】:  ${TCash + TSubsidy}\n` : ``;
      var four = `【账号总计】:  ${beans + Steel + Cash}${
        beans || Steel || Cash ? `\n` : `获取失败\n`
      }`;
      var five = `【其他总计】:  ${Subsidy + Money}${
        Subsidy || Money ? `\n` : `获取失败\n`
      }`;
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const JRSUrl = {
        ...
        body:
          "reqData=%7B%22channelSource%22%3A%22JRAPP%22%2C%22riskDeviceParam%22%3A%22%7B%7D%22%7D",
      };
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const JRDSUrl = {
        ...
        body:
          "reqData=%7B%22actCode%22%3A%22FBBFEC496C%22%2C%22type%22%3A3%2C%22riskDeviceParam%22%3A%22%22%7D",
      };
...
function JingDongPrize(s) {
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const JDkey = {
        url:
          "https://api.m.jd.com/client.action?functionId=vvipscdp_raffleAct_index&client=apple&clientVersion=8.1.0&appid=member_benefit_m",
        headers: {
          Cookie: KEY,
          ...
          if (error) {
            ...
            if (data.match(/\"raffleActKey\":\"[a-zA-z0-9]{3,}\"/)) {
              ...
              if (data.match(/(未登录|\"101\")/)) {
                ...
                merge.JDPrize.notify = `京东大奖-登录: 失败, ${
                  cc.message || `原因: 未知`
                } ⚠️`;
              }
            }
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const DollUrl = {
        ...
        body: `reqData=%7B%22actCode%22%3A%22${code}%22%2C%22type%22%3A${
          type ? type : `3`
        }%7D`,
      };
      ...
      $nobyda.post(
        {
          ...
          body: `reqData=${encodeURIComponent(
            '{"type":3,"frontParam":{"channel":"JR","belong":4},"actCode":"1DF13833F7"}'
          )}`,
        },
        (error, response, data) => {
          try {
            ...
            if (
              cc.resultCode == 0 &&
              cc.resultData.data.businessCode == "000ssq"
            ) {
              console.log(`\n京东金贴-双签签到成功 ${Details}`);
              const qt = data.match(/\"count\":\"?(\d.*?)\"?,/);
              merge.JTDouble.success = 1;
              merge.JTDouble.subsidy = qt ? qt[1] : 0;
              merge.JTDouble.notify = `京东金贴-双签: 成功, 明细: ${
                merge.JTDouble.subsidy || `无`
              }金贴 💰`;
            } else {
              console.log(`\n京东金贴-双签签到失败 ${Details}`);
              const es =
                cc.resultData && cc.resultData.data
                  ? cc.resultData.data.businessMsg
                  : null;
              const tp = data.match(/已领取|300ssq/)
                ? `已签过`
                : cc.resultCode == 3
                ? `Cookie失效`
                : `${es || cc.resultMsg || `未知`}`;
              merge.JTDouble.notify = `京东金贴-双签: 失败, 原因: ${tp}${
                cc.resultCode == 3 ? `‼️` : ` ⚠️`
              }`;
