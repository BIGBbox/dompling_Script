        ...
        try {
          if (error) {
            ...
            if (cc.resultCode == 0) {
              ...
              merge[key].notify = `${title}: 失败, 原因: ${
                cc.resultCode == 3 ? `Cookie失效‼️` : `${redata || "未知 ⚠️"}`
              }`;
              merge[key].fail = 1;
            }
          }
          ...
          (error, response, data) => {
            try {
              ...
              if (cc.msg == "success" && cc.subCode == 0) {
                ...
                console.log(`\n京东秒杀-红包签到失败 ${Details}`);
                merge.JDSecKill.fail = 1;
                merge.JDSecKill.notify = `京东秒杀-红包: 失败, 原因: ${
                  cc.subCode == 103 ? `已领取` : cc.msg ? cc.msg : `未知`
                } ⚠️`;
              }
            } catch (eor) {
              $nobyda.AnError(
      ...
      $nobyda.post(
        {
          ...
          body:
            "functionId=noahHaveFunLottery&appid=publicUseApi&body=%7B%22actId%22%3A%22RRD7PgE4k1PYkLBEJRWv6gfPkZUqgY%22%7D&client=wh5",
        },
        (error, response, data) => {
          try {
            if (error) throw new Error(error);
            ...
            if (cc.code == 0 && cc.lotteryResult) {
              ...
              merge.ElevenSale.notify = `京东入场-红包: 成功, 明细: ${
                merge.ElevenSale.Cash || `无`
              }红包 🧧${qt ? ` (2天)` : ``}`;
            } else {
              console.log(`\n京东入场-红包签到失败 ${Details}`);
              const tp =
                cc.subCode == 8
                  ...
                  ? `Cookie失效`
                  : `${cc.msg || `未知`}`;
              merge.ElevenSale.notify = `京东入场-红包: 失败, 原因: ${tp}${
                cc.code == 3 ? `‼️` : ` ⚠️`
              }`;
